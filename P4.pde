...
final float NET_BREAK_LENGTH_FACTOR = 3.0;   // Maximum distance factor (measured in number of times the rest length) allowed for the net's springs
...
final float COLLISION_BREAK_LENGTH_FACTOR = 3.0;   // Maximum distance factor (measured in number of times the rest length) allowed for the collision springs
...
void printInfo()
{
  pushMatrix();
  {
    ...
    text("Ball start velocity = " + BALL_START_VEL + " m/s", width*0.50, height*0.05);

    if (NET_IS_UNBREAKABLE)
      text("Net is unbreakable", width*0.50, height*0.075);
    else
      text("Net is breakable", width*0.50, height*0.075);
  }
  popMatrix();
}
